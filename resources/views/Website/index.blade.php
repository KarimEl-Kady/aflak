        ...
        <div class="container">
            <div class="head">
                <div class="title">
                    ...
                    <h5>{{ __('messages.what_we') }} <span>{{ __('messages.offer') }}</span> {{ __('messages.for_you') }} ?
                    </h5>
                </div>
                <div class="cards">
                    <div class="row">
                        ...
                        <div class="swiper mySwiper2">
                            <div class="swiper-wrapper">
                                @foreach ($aboutus_images as $image)
                                    <div class="swiper-slide">
                                        <img src="{{ $image->image_link ?? '' }}" alt=""
                                            style="width: 526px; height: 625px; border-radius: 10px" />
                                    </div>
                                @endforeach
                            </div>
    ...
    <div class="Services" data-aos="fade-down" data-aos-duration="2500">
        <div class="container">
            <div class="title">
                ...
                <h5>
                    {{ __('messages.learn_about') }} <span>{{ __('messages.our') }}</span>
                    {{ __('messages.most_important') }}
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <video width="100%" controls style="border-radius: 10px">
                                        <source src="{{ asset('/website/img/pexels-taryn-elliott-5309381 (1080p).mp4') }}"
                                            type="video/mp4">
                                        />
                                    </video>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="freight_type" name="type">
                                            ...
                                            @foreach ($request_type_ids as $id)
                                                @php
                                                    $request_type = \App\Models\RequestSection\RequestSectionSettingTranslation::where(
                                                        'request_section_setting_id',
                                                        $id,
                                                    )->first();
                                                @endphp
                                                @if ($request_type)
                                                    <option value="{{ $id }}">{{ $request_type->title }}
                                                    </option>
                                                @endif
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" id="load" name="load">
                                            ...
                                            @foreach ($request_load_ids as $id)
                                                @php
                                                    $request_load = \App\Models\RequestSection\RequestSectionSettingTranslation::where(
                                                        'request_section_setting_id',
                                                        $id,
                                                    )->first();
                                                @endphp
                                                @if ($request_load)
                                                    <option value="{{ $id }}">{{ $request_load->title }}
                                                    </option>
                                                @endif
                                            @endforeach
                                        </select>
                                ...
                                <div class="Updates-news">
                                    <div class="img">
                                        <a href="{{ $last_blog ? route('blog.show', $last_blog->id) : '#' }}">
                                            <img src="{{ $last_blog ? $last_blog->image_link : asset('placeholder_image.jpg') }}"
                                                alt=""
                                                style="width: 636px; height: 303px; border-radius: 10px;" />
                                        </a>
                                    </div>
                                    <h4>{{ $last_blog ? $last_blog->title : __('messages.no_blogs_found') }}</h4>
                                    <p>{{ $last_blog ? $last_blog->created_at->format('d F Y') : __('messages.no_blogs_found') }}
                                    </p>
                                        ...
                                        <div class="img">
                                            <a href="{{ route('blog.show', $blog->id) }}">
                                                <img src="{{ $blog->image_link }}" alt=""
                                                    style="width: 630px; height: 222px; border-radius: 10px;" />
                                            </a>
                                        </div>
                                        <h4>{{ $blog->title ?? '' }}</h4>
